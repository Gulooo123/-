...
**GMD 短样例(1-2小节节奏型) tempo 上限 = 145bpm**, 没有 145bpm 以上的真人律动参考!

这意味着:
- 用 --bpm 168 等高速生成时, 实际是把 ~120bpm 的真人骨架拉伸, 味道会偏机械
- **150-180bpm 的素材缺口正是 emo(中速吉他 rif) 最需要的区间**
- 这个缺口靠 LA 全集 (40万首) 补 —— 扫完按 drum_notes/n_bars + tempo 150-180 掏

**LA 下载完 → 扫出的候选池第一条规则：tempo 150-180 优先。**

---

## LA 下载损坏事故记录 (09-01)

第一次下载 9.2GB 经 hf-mirror, 因 CDN 中断导致 zip 内部数据损坏:
- 全库 404714 个 MIDI, 坏 159,291 (40%)
- 坏块跨域散布 (0 目录 0af8 起, 9 目录 6300MB 后)
- 根因: curl 长连接被 CDN 静默断流, 返回数据流错位

**修补**:
- 立即用已解压的 24.5 万好文件跑内容扫描 (立即可用)
- 重下 zip (分段续传 50MB 一片防断流)
